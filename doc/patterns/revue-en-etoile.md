...
Un artefact est soumis à N personas en parallèle, chacun le review sur son axe. Le PO consolide.

### Structure

1. Le PO identifie un artefact qui nécessite une validation multi-angle.
...
4. Le PO collecte les reviews, identifie les convergences et les contradictions, et consolide une décision.

La différence avec le pattern challenger : le challenger s'insère dans un flux de production séquentiel (le producteur intègre les retours). La revue en étoile est un mécanisme ponctuel de validation — les reviewers ne modifient pas l'artefact, le PO tranche.
...
Le PO soumet un ADR à Mira (cohérence avec l'architecture cible), Léa (rigueur formelle, références), et Marc (alignement stratégique). Chacun produit une review indépendante dans `shared/review/`. Le PO lit les trois, identifie un point de tension entre cohérence archi et stratégie, et tranche.
...
- **Paralysie** : les reviews divergent et le PO n'arrive pas à trancher.
